...
namespace Obvs.AzureServiceBus {
    internal sealed class MessagingEntityFactory : IMessagingEntityFactory {
        ...
        private MessageTypeMessagingEntityMappingDetails GetMappingDetails(Type messageType, params MessagingEntityType[] expectedEntityTypes) {
            ...
            IEnumerable<MessageTypeMessagingEntityMappingDetails> messageTypePathMappingDetailsForMessagingEntityTypes = (from mtpm in _messageTypePathMappings join eet in expectedEntityTypes on mtpm.MessagingEntityType equals eet select mtpm);
